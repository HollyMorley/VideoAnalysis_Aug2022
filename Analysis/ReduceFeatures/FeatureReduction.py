...
from Analysis.ReduceFeatures.LogisticRegression import compute_regression
...
base_save_dir = os.path.join(paths['plotting_destfolder'], f'FeatureReduction\\Round5-20250206')
...
def process_phase_comparison(mouse_id, stride_number, phase1, phase2, stride_data, condition, exp, day, base_save_dir_condition):
    ...
    # Select only runs from the two phases
    selected_mask = mask_phase1 | mask_phase2
    selected_scaled_data_df = scaled_data_df.loc[selected_mask].T

    # Run logistic regression on X (raw features)
    Xdr = np.dot(loadings_df.T, selected_scaled_data_df) # Transform X
    ...
    #full_R2, single_cvR2, unique_delta_R2, w = utils.compute_feature_importances_regression(Xdr, y_reg, selected_scaled_data_df, cv=5) #todo new/adjusted
    w, full_accuracy = compute_regression(Xdr, y_reg, selected_scaled_data_df, cv=5)
    ...
    for fidx, feature in enumerate(selected_scaled_data_df.index):
        ...
        _, single_accuracy = compute_regression(Xdr_shuffled_single, y_reg, selected_scaled_data_df, cv=5)
        _, unique_accuracy = compute_regression(Xdr_shuffled_unique, y_reg, selected_scaled_data_df, cv=5)
    ...
    # utils.plot_feature_accuracy(single_cvR2, save_path, title_suffix=f"{phase1}_vs_{phase2}")
    # utils.plot_unique_delta_accuracy(unique_delta_R2, save_path, title_suffix=f"{phase1}_vs_{phase2}")
    #
    ...
    all_trials_dr = np.dot(loadings_df.T, scaled_data_df.T)
    all_trials_dr = ((all_trials_dr.T - normalize_mean) / normalize_std).T

    run_pred = np.dot(w, np.dot(loadings_df.T, scaled_data_df.T))
    run_pred_scaled = np.dot(w, all_trials_dr)

    # plot run prediction
    utils.plot_run_prediction(scaled_data_df, run_pred, save_path, mouse_id, phase1, phase2, "")
    utils.plot_run_prediction(scaled_data_df, run_pred_scaled, save_path, mouse_id, phase1, phase2, "scaled")

    print(f"Full model accuracy: {full_accuracy:.3f}")

    # todo cross-validate regression

    # ---------------- k nearest neighbours ----------------
    # Cross validate the number of neighbours todo
    #utils.cross_validate_knn(Xdr, y_reg, save_path, mouse_id, phase1, phase2)
