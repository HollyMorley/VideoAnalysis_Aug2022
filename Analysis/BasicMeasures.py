...
class calculate_body():
    ...
    def BodyLength(self, data_chunk, stepping_limb):
...
class plotting():
    ...
    def get_discrete_measures_byrun_bystride(self, SwSt, con, mouseID, view, measure_list=['Body_length','Back_height','Back_skew']):
        ...
        levels = [np.arange(0, 42), [-3, -2, -1, 1, 2]]
        multi_index = pd.MultiIndex.from_product(levels, names=['Run', 'Stride'])
        measures = pd.DataFrame(index=multi_index,columns=measure_list)
