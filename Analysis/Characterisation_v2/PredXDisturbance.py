...
savedir = r"H:\Characterisation\PredXDisturbance"
...
apa_y_pred = [pred.y_pred[0] for pred in apa_pred if pred.stride == -1]
apa_x_vals = [np.array(list(pred.x_vals)) for pred in apa_pred if pred.stride == -1]
apa_mouse_ids = [pred.mouse_id for pred in apa_pred if pred.stride == -1]
