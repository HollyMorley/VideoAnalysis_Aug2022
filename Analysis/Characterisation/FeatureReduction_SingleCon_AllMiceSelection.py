...
def process_mouse_phase_comparison(mouse_id: str, stride_number: int, phase1: str, phase2: str,
        ...
        Tuple)[utils.PredictionData, utils.FeatureWeights, pd.DataFrame, Optional[Dict[int, any]], any, any, any, any]:
    """
...
def main(mouse_ids: List[str], stride_numbers: List[int], phases: List[str],
    ...
    if global_settings.get("overwrite_data_collection", True) or not os.path.exists(global_data_path):
        ...
        for phase1, phase2 in itertools.combinations(phases, 2):
            for stride_number in stride_numbers:
                ...
                selected_features, _ = global_feature_selection(condition_specific_settings[condition]['global_fs_mouse_ids'],
    ...
    for (phase1, phase2, stride_number), agg_data in global_data["aggregated_predictions"].items():
        top_features = utils.get_top_features(global_data["aggregated_feature_weights"], phase1, phase2, stride_number, n_features=10)
        top_feature_data = utils.get_top_feature_data(global_data["aggregated_raw_features"], phase1, phase2, stride_number, top_features)
        utils.plot_top_feature_phase_comparison(top_feature_data, base_save_dir, phase1, phase2, stride_number, condition_label=condition)
